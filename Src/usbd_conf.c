...
/* ================================================================
 * STM32G431 USB Device (PMA-based) implementation
 * ================================================================ */
#include <assert.h>
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  HAL_PCDEx_PMAConfig( pdev->pData, 0x03, PCD_SNG_BUF, 0x18 + (6 * 64) );
  HAL_PCDEx_PMAConfig( pdev->pData, 0x83, PCD_SNG_BUF, 0x18 + (7 * 64) );
...
#if defined(STM32G431xx)
/* G431 USB FS interrupt handler */
void USB_LP_IRQHandler(void) { HAL_PCD_IRQHandler(&hpcd_usb); }
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  if(HAL_PCD_Init(&hpcd_usb) != HAL_OK) { Error_Handler(); }
